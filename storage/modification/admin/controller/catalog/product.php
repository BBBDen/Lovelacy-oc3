...
class ControllerCatalogProduct extends Controller {
    ...
    protected function getList() {
        ...
        */
        ...
        foreach ($results as $result) {
            ...
            $color_result = $this->model_catalog_product->getProductColorFilter($result['product_id']);

            if ($color_result) {
                $color = $this->model_tool_image->resize($color_result['image'], 44, 44);
            } else {
                $color = '';
            }


            if ((int)$result['product_id'] === (int)$this->__get_certificate_product_id__()) {
                $product__name = 'Сертификат на покупку белья';
                $product__module = 'certificate';
                $product__price = '-####-';
            } else {
                $product__module = $result['model'];
                $product__name = $result['name'];
                $product__price = $this->currency->format($result['price'], $this->config->get('config_currency'));
            }
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['charity'])) {
            ...
            $data['charity'] = $product_info['charity'];
        } else {
            $data['charity'] = '';
        }

        if (isset($this->request->post['complect'])) {
            $data['complect'] = $this->request->post['complect'];
        } elseif (!empty($product_info)) {
            $data['complect'] = $product_info['complect'];
        } else {
            $data['complect'] = '';
        }
        ...
        $filter_groups = $this->model_catalog_product->getFilterGroups();
        ...
        if (isset($this->request->post['product_variation'])) {
            ...
            $product_variations = $this->model_catalog_product->getProductVariation($this->request->get['product_id']);
        } else {
            $product_variations = [];
        }
        ...
        if (isset($this->request->post['product_complect'])) {
            $product_complects = $this->request->post['product_complect'];
        } elseif (isset($this->request->get['product_id'])) {
            $product_complects = $this->model_catalog_product->getProductComplect($this->request->get['product_id']);
        } else {
            $product_complects = [];
        }

        $data['product_complects'] = [];

        foreach ($product_complects as $product_id) {
            $complect_info = $this->model_catalog_product->getProduct($product_id);

            if ($complect_info) {
                $data['product_complects'][] = array(
                    'product_id' => $complect_info['product_id'],
                    'name'       => $complect_info['name']
                );
            }
        }

        if (isset($this->request->post['product_complete_image'])) {
            $product_complete_images = $this->request->post['product_complete_image'];
        } elseif (isset($this->request->get['product_id'])) {
            $product_complete_images = $this->model_catalog_product->getProductCompleteImages($this->request->get['product_id']);
        } else {
            $product_complete_images = [];
        }

        $data['product_complete_images'] = [];

        foreach ($product_complete_images as $product_id) {
            $variation_info = $this->model_catalog_product->getProduct($product_id);

            if ($variation_info) {
                $data['product_complete_images'][] = array(
                    'product_id' => $variation_info['product_id'],
                    'name'       => $variation_info['name']
                );
            }
    ...
    protected function validateDelete() {
        ...
        if (in_array($this->__get_certificate_product_id__(), $this->request->post['selected'])) {
            $this->error['warning'] = 'В числе товаров, которые вы собираетесь удалить присутствует сертификат на покупку белья. Его некорректное удаление приведет к сбоям в работе системы! Чтобы деактивировать сертификат, установите ему статус "Отключено" в панели администратора.';
        }
