...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter_category'])) {
            foreach ($data['filter_category'] as $filter_category) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "filter_category SET category_id = '".(int)$filter_category."', filter_group_id = '".(int)$filter_group_id."';");
		    }
        }
	...
	public function editFilter($filter_group_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "filter_category WHERE filter_group_id = '" . (int)$filter_group_id . "'");
	...
	public function getFilterCategories($filter_group_id)
    {
        $query = $this->db->query("SELECT fc.category_id, fc.filter_group_id, cd.name FROM " . DB_PREFIX . "filter_category fc LEFT JOIN " . DB_PREFIX . "category_description cd ON (fc.category_id = cd.category_id) WHERE fc.filter_group_id = '".(int)$filter_group_id."' AND cd.language_id = '".(int)$this->config->get('config_language_id')."'");
