...
class ControllerExtensionFeedGoogleMerchantXml3 extends Controller {
	...
	public function index() {
		...
		if ($this->config->get('google_merchant_xml_3_status')) {
			$key = $this->config->get('google_merchant_xml_3_key');
	...
	public function cron() {
		...
		$filename = $dir . '/xml_feed/google_merchant_xml_3.xml';
		$filename_fb = $dir . '/xml_feed/google_merchant_xml_3_facebook.xml';
		...
		if ($this->config->get('google_merchant_xml_3_additional_images')) {
	...
	private function getXml($page = 0) {
		...
		$items_currency = $this->config->get('google_merchant_xml_3_currency');
		...
		$google_merchant_xml_3_category_google_category = $this->config->get('google_merchant_xml_3_category_google_category');
		$google_merchant_xml_3_category_product_type = $this->config->get('google_merchant_xml_3_category_product_type');
		$google_merchant_xml_3_category_condition = $this->config->get('google_merchant_xml_3_category_condition');
		$google_merchant_xml_3_category_custom_label_0 = $this->config->get('google_merchant_xml_3_category_custom_label_0');
		$google_merchant_xml_3_category_custom_label_1 = $this->config->get('google_merchant_xml_3_category_custom_label_1');
		$google_merchant_xml_3_category_custom_label_2 = $this->config->get('google_merchant_xml_3_category_custom_label_2');
		$google_merchant_xml_3_category_custom_label_3 = $this->config->get('google_merchant_xml_3_category_custom_label_3');
		$google_merchant_xml_3_category_custom_label_4 = $this->config->get('google_merchant_xml_3_category_custom_label_4');
		
		$id = $this->config->get('google_merchant_xml_3_identifier');
		$gtin = $this->config->get('google_merchant_xml_3_gtin');
		$mpn = $this->config->get('google_merchant_xml_3_mpn');
		$condition = $this->config->get('google_merchant_xml_3_condition');
		$utm = $this->config->get('google_merchant_xml_3_utm');
		$original_description = $this->config->get('google_merchant_xml_3_original_description');
		$original_images = $this->config->get('google_merchant_xml_3_original_image_status');
		$special = $this->config->get('google_merchant_xml_3_special');
		$multiplier = (float)$this->config->get('google_merchant_xml_3_multiplier') > 0 ? (float)$this->config->get('google_merchant_xml_3_multiplier') : 1;
		
		$facebook = !empty($this->request->get['target']) && $this->request->get['target'] == 'facebook';
		
		if (!empty($this->config->get('google_merchant_xml_3_category'))) {
			$categories = implode(',', $this->config->get('google_merchant_xml_3_category'));
		} else {
			$categories = false;
		}
		
		if (!empty($this->config->get('google_merchant_xml_3_manufacturer'))) {
			$manufacturers = implode(',', $this->config->get('google_merchant_xml_3_manufacturer'));
		} else {
			$manufacturers = false;
		}
		...
		foreach ($this->getProducts($categories, $manufacturers, $special, $start, $limit) as $product) {
			...
			if (!empty($google_merchant_xml_3_category_product_type[$product['category_id']])) {
				$xml .= '<g:product_type>' . $google_merchant_xml_3_category_product_type[$product['category_id']] . '</g:product_type>' . $this->eol;
			}
			...
			if (!empty($google_merchant_xml_3_category_condition[$product['category_id']])) {
				$xml .= '<g:condition>' . $google_merchant_xml_3_category_condition[$product['category_id']] . '</g:condition>' . $this->eol;
			} else {
				$xml .= '<g:condition>' . $condition . '</g:condition>' . $this->eol;
			}
			
			if (!empty($google_merchant_xml_3_category_custom_label_0[$product['category_id']])) {
				$xml .= '<g:custom_label_0>' . $google_merchant_xml_3_category_custom_label_0[$product['category_id']] . '</g:custom_label_0>' . $this->eol;
			} 

			if (!empty($google_merchant_xml_3_category_custom_label_1[$product['category_id']])) {
				$xml .= '<g:custom_label_1>' . $google_merchant_xml_3_category_custom_label_1[$product['category_id']] . '</g:custom_label_1>' . $this->eol;
			} 

			if (!empty($google_merchant_xml_3_category_custom_label_2[$product['category_id']])) {
				$xml .= '<g:custom_label_2>' . $google_merchant_xml_3_category_custom_label_2[$product['category_id']] . '</g:custom_label_2>' . $this->eol;
			} 

			if (!empty($google_merchant_xml_3_category_custom_label_3[$product['category_id']])) {
				$xml .= '<g:custom_label_3>' . $google_merchant_xml_3_category_custom_label_3[$product['category_id']] . '</g:custom_label_3>' . $this->eol;
			} 

			if (!empty($google_merchant_xml_3_category_custom_label_4[$product['category_id']])) {
				$xml .= '<g:custom_label_4>' . $google_merchant_xml_3_category_custom_label_4[$product['category_id']] . '</g:custom_label_4>' . $this->eol;
			} 
	...
	private function getProducts($categories, $manufacturers, $special = false, $start = 0, $limit = 3000) {
		...
		$customer_group = (int)$this->config->get('google_merchant_xml_3_customer_group');		
		$zero_quantity = $this->config->get('google_merchant_xml_3_zero_quantity');
		
		$min_price = (int)$this->config->get('google_merchant_xml_3_min_price');		
		$max_price = (int)$this->config->get('google_merchant_xml_3_max_price');
		$custom_xml = $this->config->get('google_merchant_xml_3_custom_sql');
