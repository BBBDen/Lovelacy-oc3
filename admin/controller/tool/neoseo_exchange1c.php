...
class ControllerToolNeoSeoExchange1c extends NeoSeoController
{
	...
	public function modeFile()
	{
		...
		if (!isset($this->request->cookie['key'])) {
			$this->log("type=catalog&mode=file: не указан сессионный ключ");
			return;
		}

		if ($this->request->cookie['key'] != md5($this->config->get('neoseo_exchange1c_password'))) {
			$this->log("type=catalog&mode=file Неверное значение куки с паролем");
			echo "failure\n";
			echo "Session error";
			return;
		}
		...
		if (isset($this->request->get['filename'])) {
			...
			$this->log("type=catalog&mode=file: не указан файл импорта");
			echo "failure\n";
