...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		foreach ($order_products as $order_product) {
			...
			if ((int)$order_product['product_id'] === $this->__get_certificate_product_id__()) {

			    $order_product_name = $language->get('certificate_text');
            } else {
			    $order_product_name = $order_product['name'];
            }
        ...
        if ( array_key_exists('promo_gift', $this->session->data) ) {
