<?php
class ControllerExtensionModuleDkSeoGenerate extends Controller {
	private $error = array();

    public function index()
    {
		$this->load->language('extension/module/dk_seo_generate');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_dk_seo_generate', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/dk_seo_generate', 'user_token=' . $this->session->data['user_token'], true)
		);

		$data['action'] = $this->url->link('extension/module/dk_seo_generate', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

		if (isset($this->request->post['module_dk_seo_generate_status'])) {
			$data['module_dk_seo_generate_status'] = $this->request->post['module_dk_seo_generate_status'];
		} else {
			$data['module_dk_seo_generate_status'] = $this->config->get('module_dk_seo_generate_status');
		}

        $this->cache->delete('dk_products');
        $this->cache->delete('dk_languages');
        $this->cache->delete('dk_category');

		$data['user_token'] = $this->session->data['user_token'];

		$data['language'] = isset($this->session->data['language']) ? $this->session->data['language'] : 'en-en';
		$this->load->model('extension/module/dk_seo_generate');

		$data['count_categories'] = $this->model_extension_module_dk_seo_generate->countCategories();
		$data['count_products'] = $this->model_extension_module_dk_seo_generate->countProducts();
		$data['count_articles'] = $this->model_extension_module_dk_seo_generate->countArticles();

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('extension/module/dk_seo_generate', $data));
	}
	...
	public function generate_urls()
    {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            $index = (int)$this->request->post['index'];
            $add_prefix = isset($this->request->post['language_prefix']) ? $this->request->post['language_prefix'] : true;

            $result = $this->{"generate_" . $type . "_url"}($index, $this->request->post['language'], $add_prefix, $this->request->post['type_empty']);
    ...
    private function generate_article_url($index, $lang, $add_prefix, $for_all = 'all')
    {
        $this->load->model('extension/module/dk_seo_generate');

        $articles = $this->cache->get('dk_articles');
        if (!$articles) {
            $articles = $this->model_extension_module_dk_seo_generate->getArticles();
            $this->cache->set('dk_articles', $articles);
        }
        $count = count($articles);
        $languages = $this->get_system_languages();

        $_language = new Language($lang);
        $_language->load($lang);
        $_language->load('extension/module/dk_seo_generate');

        $result = (isset($articles[$index])) ? $articles[$index] : [];
        $keyword = $this->get_new_url($result, $languages, $add_prefix);
        ...
        $result_url = $this->model_extension_module_dk_seo_generate->updateSeoUrl($data);

        if (((int)$index + 1) >= $count) {
            $this->cache->delete('dk_articles');
            $this->cache->delete('dk_languages');
        }
        ...
        if ($result) {
            return [
                ...
                'position' => $result,
                'url' => $result_url,
                'language' => $languages[$result['language_id']]['code']
            ];
        }
    }

    private function generate_product_url($index, $lang, $add_prefix, $for_all = 'all')
    {
        $this->load->model('extension/module/dk_seo_generate');

        // Get all products and save it to db
        $products = $this->cache->get('dk_products');
        if (!$products) {
            $products = $this->model_extension_module_dk_seo_generate->getProducts();
            $this->cache->set('dk_products', $products);
        }
        ...
        $result = (isset($products[$index])) ? $products[$index] : [];
    ...
    private function generate_category_url($index, $lang, $add_prefix, $for_all = 'all')
    {
        $this->load->model('extension/module/dk_seo_generate');

        // Get Categories and Save it to cache
        $categories = $this->cache->get('dk_category');
        if (!$categories) {
            $categories = $this->model_extension_module_dk_seo_generate->getCategories();
            $this->cache->set('dk_category', $categories);
        }
        $count = count($categories);

        // Get languages for current site and save it to cache
        $languages = $this->get_system_languages();
        ...
        $result = (isset($categories[$index])) ? $categories[$index] : [];

        if ($result) {

            $result_string = $this->get_new_url($result, $languages, $add_prefix);


            $data = [
                'language_id' => $result['language_id'],
                'store_id' => $this->request->post['store_id'],
                'keyword' => $result_string,
                'id' => $result['id'],
                'query' => 'category_id',
                'all' => $for_all,
                'type' => 'category'
    ...
    {
		if (!$this->user->hasPermission('modify', 'extension/module/dk_seo_generate')) {
	...
	private function get_new_url($result, $languages, $add_prefix)
    {
        $split_string = str_replace($this->get_not_allowed_symbols(), '', mb_str_split(trim($result['name'], ' ')));
        $lang = $this->dk_get_locale($languages[$result['language_id']]['code']);
        $str_new_split = [];
        ...
        $prefix = 'true' === $add_prefix ? '-' . $languages[$result['language_id']]['code'] : '';

        return $str_new_split ? strtolower(str_replace(' ', '-', implode('', $str_new_split)) . $prefix) : '';
    }

    private function get_not_allowed_symbols()
    {
        return [
            '&',
            '№',
    ...
    private function get_system_languages()
    {
        $this->load->model('localisation/language');
        $languages = $this->cache->get('dk_languages');
        if (!$languages) {
            $_languages_ = $this->model_localisation_language->getLanguages();
            $languages = [];
            if ($_languages_) {
                foreach ($_languages_ as $key => $value) {
                    $languages[$value['language_id']] = [
                        'code' => explode('-', $key)[0]
                    ];
                }
                $this->cache->set('dk_languages', $languages);
            }
        }
    ...
    public function dk_get_locale( $locale = '' ) {
        switch ( $locale ) {
            // Belorussian.
            case 'by':
                $table = [
