		...
		if ($this->config->get('remarketing_status')) {
			...
			if (!$this->model_tool_remarketing->isBot()) {
				...
				/*if ($this->config->get('remarketing_google_status') && $this->config->get('remarketing_google_gtag_status')) {
					...
					$data['output'] .= 'gtag(\'config\', \'' . $this->config->get('remarketing_google_identifier') . '\',{\'allow_enhanced_conversions\':true});' . "\n";
					$data['output'] .= '</script>' . "\n";
				}*/
		...
		if ($this->config->get('remarketing_status') && !$this->model_tool_remarketing->isBot()) {
			...
			switch ($route) {
				...
				case 'lightcheckout/checkout':
					...
					$data['facebook_totalvalue'] = $this->currency->format($cart_total * (float)$this->config->get('remarketing_facebook_ratio'), $facebook_currency, '', false); 
					$data['ecommerce_totalvalue'] = $this->currency->format($cart_total * (float)$this->config->get('remarketing_ecommerce_ratio'), $ecommerce_currency, '', false); 
					...
					if ($this->config->get('remarketing_ecommerce_gtag_status')) {
						$data['ecommerce_output'] .= '<script>' . "\n";
						$data['ecommerce_output'] .= "if (typeof gtag != 'undefined') {" . "\n";
						$data['ecommerce_output'] .= 'gtag("event", "begin_checkout" , {'."\n";
						$data['ecommerce_output'] .= "'send_to': '" . $this->config->get('remarketing_ecommerce_analytics_id') . "',\n";
						$data['ecommerce_output'] .= "'currency': '" . $ecommerce_currency . "',\n";
						$data['ecommerce_output'] .= "'value': " . $data['ecommerce_totalvalue'] . ",\n";
						$data['ecommerce_output'] .= "'items': ["."\n";
						foreach ($products as $product) {
				...
				case 'extension/payment/sbacquiring/success':  
				case 'extension/ocdevwizard/smart_order_success_page_pro': 
				case 'extension/payment/tinkoff/success': 
					...
					if (!empty($this->request->cookie['remarketing_order_id']) || !empty($this->session->data['order_id']) || !empty($this->session->data['remarketing_order_id'])) {
						...
						if ($order_info) {
							...
							$data['google_totalvalue'] = $order_info['google_total']; 
							$data['google_conversion_totalvalue'] = $order_info['google_conversion_total']; 
							$data['facebook_totalvalue'] = $order_info['facebook_total']; 
							$data['ecommerce_totalvalue'] = $order_info['ecommerce_total'];
						    ...
						    if ($this->config->get('remarketing_google_status') && $this->config->get('remarketing_google_identifier')) {
								$data['google_output'] .= '<script>var enhanced_conversion_data = {' . "\n";
								if ($order_info['order_info']['email']) {
									$data['google_output'] .= '"email": "' . $order_info['order_info']['email'] . '",' . "\n";
								}
								if ($order_info['order_info']['telephone']) {
									$data['google_output'] .= '"phone_number": "' . $order_info['order_info']['telephone'] . '",' . "\n";
								}
								if ($order_info['order_info']['firstname']) {
									$data['google_output'] .= '"first_name": "' . $order_info['order_info']['firstname'] . '",' . "\n";
								}
								if ($order_info['order_info']['lastname']) {
									$data['google_output'] .= '"last_name": "' . $order_info['order_info']['lastname'] . '",' . "\n";
								}
								if ($order_info['order_info']['shipping_address_1']) {
									$data['google_output'] .= ' "home_address": {';
									$data['google_output'] .= '"street": "' . $order_info['order_info']['shipping_address_1'] . '",' . "\n";
									$data['google_output'] .= '"city": "' . $order_info['order_info']['shipping_city'] . '",' . "\n";
									$data['google_output'] .= '"region": "' . $order_info['order_info']['shipping_zone'] . '",' . "\n";
									$data['google_output'] .= '"country": "' . $order_info['order_info']['shipping_country'] . '"' . "\n";
									$data['google_output'] .= '}';  
								}
								$data['google_output'] .= '}</script>' . "\n";
							}
							...
							if ($this->config->get('remarketing_google_ads_identifier')) {
								...
								$data['google_output'] .= "'currency': '" . $google_currency . "',\n";
								$data['google_output'] .= "'transaction_id': '" . $order_info['order_id'] . "'\n";
								$data['google_output'] .= '})};' . "\n</script>\n";
							}
							...
							if ($this->config->get('remarketing_ecommerce_status')) {		
								...
								$data['ecommerce_output'] .= "'affiliation': '" . addslashes($order_info['store_name']) . "'," . "\n";
							...
							if ($this->config->get('remarketing_ecommerce_gtag_status')) {
								...
								foreach ($order_info['products'] as $product) {
									$data['ecommerce_output'] .= "{"."\n";
									$data['ecommerce_output'] .= "'id': '" . ($this->config->get('remarketing_ecommerce_ga4_id') == 'id' ? $product['product_id'] : $product['model']) . "'," . "\n";
									$data['ecommerce_output'] .= "'name': '" . addslashes($product['name']) . "'," . "\n";
									$data['ecommerce_output'] .= "'list_name': '" . addslashes($product['category']) . "'," . "\n";
									if (!empty($product['product_info']['manufacturer'])) $data['ecommerce_output'] .= "'brand': '" . addslashes($product['product_info']['manufacturer']) . "'," . "\n";
									if (!empty($product['category'])) $data['ecommerce_output'] .= "'category': '" . addslashes($product['category']) . "'," . "\n";
									if (!empty($product['variant'])) $data['ecommerce_output'] .= "'variant': '" . addslashes($product['variant']) . "'," . "\n";
									$data['ecommerce_output'] .= "'quantity': " . $product['quantity'] . ","."\n";
									$data['ecommerce_output'] .= "'price': " . $product['ecommerce_price'] . "},\n";
								}
			...
			if ($this->config->get('remarketing_tiktok_status')) {
				if ($data['tiktok_page'] == 'purchase' || $data['tiktok_page'] == 'initiate') {
					if (!empty($data['tiktok_ids'])) {
						...
						if ($data['facebook_page'] == 'purchase') {
							$data['tiktok_output'] .= "ttq.track('CompletePayment', {" . "\n";
						} else {
							$data['tiktok_output'] .= "ttq.track('InitiateCheckout', {" . "\n";	
						}
			...
			if ($data['google_reviews_page'] && $this->config->get('remarketing_reviews_status') && $this->config->get('remarketing_google_merchant_identifier')  && strpos($data['reviews_order_email'], 'localhost') === false) {			
				...
				if (!empty($items)) {
					$data['google_reviews_output'] .= ",\n";
					$data['google_reviews_output'] .= '"products": ' . json_encode($items);
				}
			...
			if ($this->config->get('remarketing_events_quick_purchase')) {
				...
				$data['events_output'] .= html_entity_decode($remarketing_events_quick_purchase);
				$data['events_output'] .= "\n}\n";     
				$data['events_output'] .= "</script>\n";     
			}
	...
	public function sendMeasurementManual() {
		...
		if (!empty($this->request->post)) {
			if (!empty($this->request->post['manual_id'])) {
				...
				if (!empty($this->request->post['manual_cn'])) {
					$ecommerce_data['cn'] = $this->request->post['manual_cn'];
				}
				if (!empty($this->request->post['manual_cs'])) {
					$ecommerce_data['cs'] = $this->request->post['manual_cs'];
				}
				if (!empty($this->request->post['manual_cm'])) {
					$ecommerce_data['cm'] = $this->request->post['manual_cm'];
				}
