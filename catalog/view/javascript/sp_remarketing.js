...
function sendFacebookCategoryDetails(data, search_page) {
	if (data) {
		...
		data: {products: data['products'], time: data['time'], url: window.location.href, search: search_page},
			dataType: 'json',
            success: function(json) {
...
function sendWishList(json) {
	...
	if (typeof heading == 'undefined' || heading == '') {
		...
		if (json['remarketing']['tiktok_status'] != '0') {
			if (typeof ttq != 'undefined') {
				ttq.track('AddToWishlist', json['remarketing']['tiktok_event']); 
			}
		}
...
$(document).ready(function() {
	console.log ('%c%s', 'color: green; font: 1.2rem/1 Tahoma;', 'sp remarketing 5.5 start');
