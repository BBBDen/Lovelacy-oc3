	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				if ((int)$product['product_id'] === (int)$this->__get_certificate_product_id__()) {
				    $product__name = 'Подарочный сертификат';
                } else {
            ...
            $data['coupon_history'] = [];
            $coupon_history = $this->model_marketing_coupon->getCouponByOrderId($order_info['order_id']);
            $this->load->model('tool/image');
            $products = [];

            foreach ($coupon_history['products'] as $prod) {
                $products[] = [
                    'name' => $prod['name'],
                    'image' => $this->model_tool_image->resize($prod['image'], 100, 100)
                ];
            }
            $data['coupon_history'] = [
