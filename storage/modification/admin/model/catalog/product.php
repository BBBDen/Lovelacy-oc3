...
class ModelCatalogProduct extends Model {
    public function addProduct($data) {
        ...
        foreach ($data['product_description'] as $language_id => $value) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', seo_description = '" . $this->db->escape($value['seo_description']) . "', sale_description = '". $this->db->escape($value['sale_description']) ."', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
        }
            ...
            foreach ($data['product_option'] as $product_option) {
                if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                    if (isset($product_option['product_option_value'])) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "', filter_group_id = '" . (int)$product_option['filter_group_id'] . "'");
        ...
        if (isset($data['product_variation'])) {
            foreach ($data['product_variation'] as $variation_id) {
                $this->db->query("DELETE FROM " . DB_PREFIX . "product_variation WHERE product_id = '" . (int)$product_id . "' AND product_variation_id = '" . (int)$variation_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_variation SET product_id = '" . (int)$product_id . "', product_variation_id = '" . (int)$variation_id . "'");
            }
        }

        if (isset($data['product_complect'])) {
            foreach ($data['product_complect'] as $complect_id) {
                $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_complect WHERE product_id = '" . (int)$product_id . "' AND complect_id = '" . (int)$complect_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_complect SET product_id = '" . (int)$product_id . "', complect_id = '" . (int)$complect_id . "'");
            }
        }

        if (isset($data['product_complete_image'])) {
            foreach ($data['product_complete_image'] as $complete_id) {
                $this->db->query("DELETE FROM " . DB_PREFIX . "product_complete_image WHERE product_id = '" . (int)$product_id . "' AND complete_image_id = '" . (int)$complete_id . "'");
                $this->db->query("INSERT INTO " . DB_PREFIX . "product_complete_image SET product_id = '" . (int)$product_id . "', complete_image_id = '" . (int)$complete_id . "'");
            }
        }
    ...
    public function editProduct($product_id, $data) {
        ...
        if (isset($data['product_option'])) {
            foreach ($data['product_option'] as $product_option) {
                if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                    if (isset($product_option['product_option_value'])) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_option_id = '" . (int)$product_option['product_option_id'] . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', required = '" . (int)$product_option['required'] . "', filter_group_id = '" . (int)$product_option['filter_group_id'] . "'");
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_to_complect WHERE product_id = '" . (int)$product_id . "'");
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_complete_image WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function copyProduct($product_id) {
        ...
        if ($query->num_rows) {
            ...
            $data['product_complect'] = $this->getProductComplect($product_id);
            $data['product_complect_image'] = $this->getProductCompleteImages($product_id);
    ...
    public function getProductDescriptions($product_id) {
        ...
        foreach ($query->rows as $result) {
            $product_description_data[$result['language_id']] = array(
                ...
                'sale_description' => $result['sale_description'],
    ...
    {
        $query = $this->db->query("SELECT fg.filter_group_id, fgd.name as filter_name FROM " . DB_PREFIX . "filter_group as fg LEFT JOIN " . DB_PREFIX . "filter_group_description as fgd ON (fg.filter_group_id = fgd.filter_group_id) WHERE fgd.language_id = '". (int)$this->config->get('config_language_id') ."'");
    ...
    public function getProductVariation($product_id) {
        ...
        foreach ($query->rows as $result) {
            $product_variation_data[] = $result['product_variation_id'];
        }
    ...
    public function getProductComplect($product_id) {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_complect WHERE product_id = '" . (int)$product_id . "'");

        foreach ($query->rows as $result) {
            $product_variation_data[] = $result['complect_id'];
        }

        return $product_variation_data;
    }

    public function getProductCompleteImages($product_id) {
        $product_variation_data = array();

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_complete_image WHERE product_id = '" . (int)$product_id . "'");

        foreach ($query->rows as $result) {
            $product_variation_data[] = $result['complete_image_id'];
        }
