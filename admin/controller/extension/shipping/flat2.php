...
class ControllerExtensionShippingFlat2 extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/shipping/flat2', 'user_token=' . $this->session->data['user_token'], true)
		);
		...
		if (isset($this->request->post['shipping_flat2_cost'])) {
			$data['shipping_flat_cost'] = $this->request->post['shipping_flat2_cost'];
		} else {
			$data['shipping_flat_cost'] = $this->config->get('shipping_flat2_cost');
		}
		...
		if (isset($this->request->post['shipping_flat2_status'])) {
			$data['shipping_flat_status'] = $this->request->post['shipping_flat2_status'];
		} else {
			$data['shipping_flat_status'] = $this->config->get('shipping_flat2_status');
		}

		if (isset($this->request->post['shipping_flat2_sort_order'])) {
			$data['shipping_flat_sort_order'] = $this->request->post['shipping_flat2_sort_order'];
		} else {
			$data['shipping_flat_sort_order'] = $this->config->get('shipping_flat2_sort_order');
		}
