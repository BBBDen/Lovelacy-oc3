...
class ModelToolRemarketing extends Model {
	public function sendEcommerce($ecommerce_data) {
		...
		if (!$this->isBot() && !empty($this->session->data['uuid'])) {
			
		$ecommerce_data['v'] = 1;	
		$ecommerce_data['tid'] = $this->config->get('remarketing_ecommerce_analytics_id');
		$ecommerce_data['cid'] = $this->session->data['uuid'];	
		
		if ($this->customer->isLogged() && $this->config->get('remarketing_ecommerce_measurement_user_id')) {
			$ecommerce_data['uid'] = $this->customer->isLogged();
			unset($ecommerce_data['cid']);
		}
		
		$ecommerce_data['t'] = 'event';	
		...
		if (!empty($ecommerce_data['ip'])) {
			$ecommerce_data['uip'] = $ecommerce_data['ip'];
		}
		...
		if (!empty($ecommerce_data['user_agent'])) {
            $ecommerce_data['ua'] = $ecommerce_data['user_agent'];
            $user_agent = $ecommerce_data['user_agent'];
        }
		...
		if (!empty($this->session->data['utm_source']) && empty($ecommerce_data['cs'])) {
            $ecommerce_data['cs'] = $this->session->data['utm_source'];
        }
		if (!empty($this->session->data['utm_medium']) && empty($ecommerce_data['cm'])) {
		...
		if (!empty($this->session->data['utm_campaign']) && empty($ecommerce_data['cn'])) {
	...
	public function getQuickOrderSuccess($order_id) {
		...
		if ($order_info) {
			...
			$json['remarketing']['reviews_products'] = [];
			if ($order_info['products']) {
				...
				foreach ($order_info['products'] as $product) {
					...
					if (!empty($product['product_info'][$this->config->get('remarketing_reviews_feed_gtin')])) {
						$json['remarketing']['reviews_products'][] = ['gtin' => $product['product_info'][$this->config->get('remarketing_reviews_feed_gtin')]];	
					}
			...
			$json['remarketing']['google_gtag_event'] = [
				'send_to' => $this->config->get('remarketing_ecommerce_analytics_id'),
				'value'   => $order_info['ecommerce_total'],
				'transaction_id'   => $json['remarketing']['order_info']['order_id'],
				'currency'   => $json['remarketing']['ecommerce_currency'],
				'affiliation'    => $order_info['store_name'],
				'items' => $json['remarketing']['ecommerce_products']
			];
